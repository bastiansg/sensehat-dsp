...
# NOTE ref: https://github.com/virtualabs/pixoo-client
class Pixoo(object):
    ...
    def send(self, cmd, args):
        ...
        if self.btsock is not None:
            _ = self.btsock.send(bytes(spp_frame))
    ...
    def encode_image(self, image: Image.Image):
        encoded_image = self.encode_raw_image(image)
    ...
    def draw_pic(self, image: Image.Image):
        """
        Draw encoded picture.
        """
        nb_colors, palette, pixel_data = self.encode_image(image)
        frame_size = 7 + len(pixel_data) + len(palette)
        frame_header = [
            0xAA,
            frame_size & 0xFF,
...
class PixooMax(Pixoo):
    ...
    def encode_image(self, filepath):
        ...
        img = img.convert(
            mode="P", palette=Image.ADAPTIVE, colors=256
        ).convert(mode="RGB")
